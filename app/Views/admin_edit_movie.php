...
<form action="/admin/edit_movie/<?= $movie->movie_id ?>" method="post">
    ...
    <div class="mb-4">
        ...
        <input type="text" name="movie_duration" id="movie_duration" value="<?= $movie->movie_duration ?>" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2">
    </div>
    <div class="mb-4">
        <label for="price" class="block text-sm font-medium text-gray-700">Price (THB)</label>
        <input type="text" name="price" id="price" value="<?= $movie->price ?>" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2">
    </div>
